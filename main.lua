...
local function RebuildESP()
    ...
    local anyActive = State.BoxESP or State.SkeletonESP or State.Tracer
                   or State.HealthBarESP or State.DistanceESP or State.NameESP or State.Chams
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        if State.Chams then
            ...
            hl.Adornee          = char
            hl.Color3           = Color3.fromRGB(220, 50, 50)
            hl.SurfaceColor3    = Color3.fromRGB(220, 50, 50)
        ...
        if State.NameESP or State.HealthBarESP or State.DistanceESP then
            ...
            if State.NameESP then
                local nl = NewInstance("TextLabel", {
            ...
            if State.HealthBarESP then
                local hum = char:FindFirstChildOfClass("Humanoid")
                local hl2 = NewInstance("TextLabel", {
            ...
            if State.DistanceESP then
                local dl = NewInstance("TextLabel", {
                    Size = UDim2.new(1,0,0,14),
                    BackgroundTransparency = 1,
                    Text = "0 studs",
                ...
                Track(RunService.RenderStepped:Connect(function()
                    ...
                    if lhrp and hrp and hrp.Parent then
                        dl.Text = math.floor((lhrp.Position - hrp.Position).Magnitude) .. " studs"
                    end
                end))
            end
        end
...
for _, t in ipairs({
    { key = "BoxESP",       label = "Box ESP",          desc = "Draw a bounding box around each player" },
    { key = "SkeletonESP",  label = "Skeleton ESP",     desc = "Visualize player joint lines" },
    { key = "Tracer",       label = "Tracer Lines",     desc = "Draw lines from screen centre to players" },
    { key = "HealthBarESP", label = "Health Bar & HP",  desc = "Show current HP above the player" },
    { key = "DistanceESP",  label = "Distance",         desc = "Show distance in Roblox studs" },
    { key = "NameESP",      label = "Name & Tool",      desc = "Show username and held tool" },
    { key = "Chams",        label = "Chams / Highlight", desc = "Colour players through walls" },
}) do
    local k = t.key
    MakeToggle(VisualsPanel, {
        ...
        description = t.desc,
        default     = false,
        onToggle    = function(on) State[k] = on; RebuildESP() end,
    })
end
